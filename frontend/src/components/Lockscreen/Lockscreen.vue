  ...
  >
    ...
    <template v-if="showLogin">
      <div class="login-box">
        ...
        <div class="username">{{ loginParams.user }}</div>
        <n-input
          type="password"
          autofocus
          v-model:value="loginParams.pwd"
          @keyup.enter="onLogin"
          placeholder="请输入登录密码"
        >
          <template #suffix>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const onLogin = async () => {
        ...
        const { errno:code,errmsg: message } = await userStore.login(params);
